...
When the backend answers that the selected model is at capacity, the runtime proxy waits and re-sends the same request instead of rotating. Capacity is a property of the model, not of an account, so rotating spends the pool's transient budget against accounts that all fail identically and the request ends as a 503 within seconds. The wait backs off (2s, 5s, 15s, 30s, then 60s), honors an upstream `retry-after` when one is sent, and stops at `CODEX_MULTI_AUTH_MODEL_CAPACITY_RETRY_MS` (10 minutes by default). Set that variable to `0` to restore the previous rotate-and-fail behavior. The account is never marked rate limited or cooled down for a capacity response. See [#689](https://github.com/ndycode/codex-multi-auth/issues/689).
